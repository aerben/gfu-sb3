...
class PersonControllerTest {
    ...
    void getPersonById() {
        ...
        webClient.get().uri("/people/{id}", person1.getId())
    ...
    void getPeople() {
        ...
        when(this.personRepository.findAll()).thenReturn(Flux.just(person1, person2, person3));
        webClient.get().uri("/people")
            .header(HttpHeaders.ACCEPT, "application/json")
            .exchange()
            .expectStatus().isOk()
            .expectBodyList(Person.class).contains(person1, person2, person3);
